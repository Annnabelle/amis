...
const Session = () => {
 const { t } = useTranslation();
  ...
  const [timeLeft, setTimeLeft] = useState<number>(() => {
    ...
    return SESSION_DURATION; // если нет — стартуем с часа
  });

  useEffect(() => {
    let sessionEnd = localStorage.getItem("sessionEnd");

    if (!sessionEnd) {
      const endTime = Date.now() + SESSION_DURATION * 1000;
      localStorage.setItem("sessionEnd", String(endTime));
      sessionEnd = String(endTime);
    }

    const endTimestamp = parseInt(sessionEnd);
