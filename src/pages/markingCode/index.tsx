...
const MarkingCode = () => {
    ...
    const handleRowClick = (type: 'MarkingCode', action: 'retrieve' | 'edit' | 'delete', record: MarkingCodeTableDataType) => {
        ...
        if (type === "MarkingCode" && action === "retrieve") {
            navigate(`/marking-codes/${id}/product/${record.key}`); //${record.key}
        }
    };
  return (
    <MainLayout>
        <Heading 
            title={t('markingCodes.batches.title')}
            extraSubtitles={{
                orderNumber: {
                    title: `${t('markingCodes.batches.orderNumber')}:`,
                    value: `${markingCodeById?.poNumber || ''}`
                },
                orderTime: {
                ...
                turonOrderID: {
                    ...
                    value: `${markingCodeById?.orderId || ''}`
                }
            }}
        >
            <CustomButton className='outline' onClick={() => navigate(`/marking-codes`)}>{t("markingCodes.backToOrders")}</CustomButton>
        </Heading>
        <div className="box">
